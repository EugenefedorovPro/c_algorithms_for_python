#include <CUnit/Basic.h>
#include <CUnit/CUnit.h>
#include <CUnit/TestDB.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>

#include "../declarations.h"

void test_remove_rbt() {
    int is_test_all = 1;

    /* // NO TREE BALANCING */
    /* // case 1 - check remove node in the left subtree */
    /* // case 1.1 - node to rm has two sub */
    /* // case 1.1.1 - left child of node_to_rm has two children */

    Node *root = NULL;

    /* size_t number_nodes = 31; */
    /* size_t number_trees = 1; */
    /* size_t count = 0; */
    /* int *arr_rand_numbers = rand_numbers(number_nodes); */
    /* int n_fails = check_inserts_in_multiple_trees(&root, &arr_rand_numbers, number_nodes,
     * number_trees, &count); */
    /* CU_ASSERT(n_fails == 0); */

    /*  // ************************* RB TREE BALANCE *1/ **************************** */

    char *issue = NULL;
    if (is_test_all) {
        printf("\n\n----------------------case_1---------------------------\n\n");
        // red leaf node
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 2, "empty");
        insert_rbt(&root, 9, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        insert_rbt(&root, 38, "empty");
        remove_rbt(&root, 38);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->right->key == 30);
        CU_ASSERT(root->right->right->key == 40);
        CU_ASSERT(root->right->left->key == 25);
        CU_ASSERT(root->left->left->key == 2);
        CU_ASSERT(root->left->right->key == 9);
        CU_ASSERT(root->right->right->left == NULL);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        /* free_tree(root); */

        // DB sibling's black and  DB's sibling's children are black (or null)
        // remove left sibling
        printf("\n\n----------------------case_2---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        root->right->color = RED;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == RED);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB sibling's black and  DB's sibling's children are black (or null) and parent is red
        // remove right sibling
        // node_to_rm has right child
        printf("\n\n----------------------case_3---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 31, "empty");
        root->right->color = RED;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        remove_rbt(&root, 30);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == RED);
        CU_ASSERT(root->right->left->key == 15);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->right->right->key == 31);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        /* /1* free_tree(root); *1/ */

        // DB sibling's black and  DB's sibling's children are black (or null) and parent is red
        // remove right sibling
        // node_to_rm has right child
        printf("\n\n----------------------case_4---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 29, "empty");
        root->right->color = RED;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        remove_rbt(&root, 30);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == RED);
        CU_ASSERT(root->right->left->key == 15);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->right->right->key == 29);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        /* free_tree(root); */

        // DB sibling's black and  DB's sibling's children are black (or null)
        printf("\n\n----------------------case_5---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == RED);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 7);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == RED);
        CU_ASSERT(root->right->left == NULL);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm left node of a parent
        printf("\n\n----------------------case_6---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = RED;
        root->right->right->right->color = BLACK;
        root->right->right->left->color = BLACK;
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 7);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->key == 30);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 40);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 20);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->right->left->right->key == 25);
        CU_ASSERT(root->right->left->right->color == RED);
        CU_ASSERT(root->right->left->left == NULL);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        /* free_tree(root); */

        printf("\n\n----------------------case_7---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -3, "empty");
        insert_rbt(&root, -5, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, -2, "empty");
        insert_rbt(&root, 3, "empty");
        insert_rbt(&root, 0, "empty");
        remove_rbt(&root, -3);
        CU_ASSERT(root->key = -4);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm left node of a parent
        // node_to_del has right child
        printf("\n\n----------------------case_8---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = RED;
        root->right->right->right->color = BLACK;
        root->right->right->left->color = BLACK;
        insert_rbt(&root, 16, "empty");
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 7);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == RED);
        CU_ASSERT(root->right->left->key == 16);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->right->right->left->key == 25);
        CU_ASSERT(root->right->right->left->color == BLACK);
        CU_ASSERT(root->right->right->right->key == 40);
        CU_ASSERT(root->right->right->right->color == BLACK);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm left node of a parent
        // node_to_del has left child
        printf("\n\n----------------------case_9---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = RED;
        root->right->right->right->color = BLACK;
        root->right->right->left->color = BLACK;
        insert_rbt(&root, 14, "empty");
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 7);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == RED);
        CU_ASSERT(root->right->right->right->key == 40);
        CU_ASSERT(root->right->right->right->color == BLACK);
        CU_ASSERT(root->right->right->left->key == 25);
        CU_ASSERT(root->right->right->left->color == BLACK);
        CU_ASSERT(root->right->left->key == 14);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm left node of a parent
        // node_to_del has two children
        printf("\n\n----------------------case_10---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = RED;
        root->right->right->right->color = BLACK;
        root->right->right->left->color = BLACK;
        insert_rbt(&root, 14, "empty");
        insert_rbt(&root, 16, "empty");
        remove_rbt(&root, 15);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 7);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == RED);
        CU_ASSERT(root->right->left->key == 14);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->right->left->right->key == 16);
        CU_ASSERT(root->right->left->right->color == RED);
        CU_ASSERT(root->right->right->left->key == 25);
        CU_ASSERT(root->right->right->left->color == BLACK);
        CU_ASSERT(root->right->right->right->key == 40);
        CU_ASSERT(root->right->right->right->color == BLACK);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm right node of a parent
        // node_to_del has no children
        printf("\n\n----------------------case_11---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = BLACK;
        root->right->color = BLACK;
        root->right->left->color = BLACK;
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, -1, "empty");
        root->left->left->color = RED;
        root->left->left->left->color = BLACK;
        root->left->left->right->color = BLACK;
        remove_rbt(&root, 7);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->left->key == 0);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->left->left->key == -1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 5);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 15);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->left->right->left->key == 1);
        CU_ASSERT(root->left->right->left->color == RED);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // node_to_del has right child
        printf("\n\n----------------------case_12---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = BLACK;
        root->right->color = BLACK;
        root->right->left->color = BLACK;
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, -1, "empty");
        root->left->left->color = RED;
        root->left->left->left->color = BLACK;
        root->left->left->right->color = BLACK;
        insert_rbt(&root, 8, "empty");
        remove_rbt(&root, 7);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->left->left->key == 0);
        CU_ASSERT(root->left->left->color == RED);
        CU_ASSERT(root->left->left->left->key == -1);
        CU_ASSERT(root->left->left->left->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 15);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->left->left->right->key == 1);
        CU_ASSERT(root->left->left->right->color == BLACK);
        CU_ASSERT(root->left->right->key == 8);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm right node of a parent
        // node_to_del has left child
        printf("\n\n----------------------case_13---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 15, "empty");
        insert_rbt(&root, 30, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = BLACK;
        root->right->color = BLACK;
        root->right->left->color = BLACK;
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, -1, "empty");
        root->left->left->color = RED;
        root->left->left->left->color = BLACK;
        root->left->left->right->color = BLACK;
        insert_rbt(&root, 6, "empty");
        remove_rbt(&root, 7);
        CU_ASSERT(root->key == 10);
        CU_ASSERT(root->left->left->key == 0);
        CU_ASSERT(root->left->left->color == RED);
        CU_ASSERT(root->left->left->left->key == -1);
        CU_ASSERT(root->left->left->left->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->key == 20);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 15);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->left->left->right->key == 1);
        CU_ASSERT(root->left->left->right->color == BLACK);
        CU_ASSERT(root->left->right->key == 6);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm right node of a parent
        // node_to_del has right child
        printf("\n\n----------------------case_14---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        insert_rbt(&root, 20, "empty");
        insert_rbt(&root, 28, "empty");
        root->left->left->color = BLACK;
        root->left->right->color = BLACK;
        root->right->color = BLACK;
        root->right->right->color = BLACK;
        root->right->left->color = RED;
        root->right->left->left->color = BLACK;
        root->right->left->right->color = BLACK;
        remove_rbt(&root, 1);
        CU_ASSERT(root->key == 25);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->left->key == 10);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->left->left->key == 5);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 20);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->key == 30);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->right->right->key == 40);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 28);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->left->left->right->key == 7);
        CU_ASSERT(root->left->left->right->color == RED);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // DB's sibling is red
        // rm right node of a parent
        // node_to_del has right child
        printf("\n\n----------------------case_15---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        insert_rbt(&root, 8, "empty");
        insert_rbt(&root, 6, "empty");
        root->left->color = BLACK;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        root->left->right->color = RED;
        root->left->right->left->color = BLACK;
        root->left->right->right->color = BLACK;
        /* insert_rbt(&root, 50); */
        /* insert_rbt(&root, 35); */
        remove_rbt(&root, 40);
        CU_ASSERT(root->key == 7);
        CU_ASSERT(root->color == BLACK);
        CU_ASSERT(root->right->key == 10);
        CU_ASSERT(root->right->color == BLACK);
        CU_ASSERT(root->left->key == 5);
        CU_ASSERT(root->left->color == BLACK);
        CU_ASSERT(root->right->right->key == 30);
        CU_ASSERT(root->right->right->color == BLACK);
        CU_ASSERT(root->right->left->key == 8);
        CU_ASSERT(root->right->left->color == BLACK);
        CU_ASSERT(root->left->left->key == 1);
        CU_ASSERT(root->left->left->color == BLACK);
        CU_ASSERT(root->left->right->key == 6);
        CU_ASSERT(root->left->right->color == BLACK);
        CU_ASSERT(root->right->right->left->key == 25);
        CU_ASSERT(root->right->right->left->color == RED);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // remove root */
        // root has no children
        printf("\n\n----------------------case_16---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        remove_rbt(&root, 10);
        CU_ASSERT(root == NULL);
        free_tree(root);

        // remove root */
        // root has one left child
        printf("\n\n----------------------case_17---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        remove_rbt(&root, 10);
        CU_ASSERT(root->key == 5);
        CU_ASSERT(root->color == BLACK);
        free_tree(root);

        // remove root */
        // root has one right child
        printf("\n\n----------------------case_18---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 20, "empty");
        remove_rbt(&root, 10);
        CU_ASSERT(root->key == 20);
        CU_ASSERT(root->color == BLACK);
        free_tree(root);

        // remove root
        // root has two children
        printf("\n\n----------------------case_19---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        insert_rbt(&root, 8, "empty");
        insert_rbt(&root, 6, "empty");
        root->left->color = BLACK;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        root->left->right->color = RED;
        root->left->right->left->color = BLACK;
        root->left->right->right->color = BLACK;
        remove_rbt(&root, 10);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // sibling's children are both red
        // remove not root
        printf("\n\n----------------------case_20---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -3, "empty");
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 2, "empty");
        remove_rbt(&root, -4);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // sibling's children are both red
        // remove root
        printf("\n\n----------------------case_21---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -3, "empty");
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 2, "empty");
        remove_rbt(&root, -3);
        CU_ASSERT(root->key == 1);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // sibling's children are both red
        // remove leaf remove root
        printf("\n\n----------------------case_22---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -3, "empty");
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 2, "empty");
        remove_rbt(&root, 2);
        remove_rbt(&root, -3);
        CU_ASSERT(root->key == 0);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        // remove root
        // max node to remove has left child
        printf("\n\n----------------------case_23---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -2, "empty");
        insert_rbt(&root, -3, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, -1, "empty");
        insert_rbt(&root, 2, "empty");
        insert_rbt(&root, -5, "empty");
        remove_rbt(&root, -2);
        CU_ASSERT(root->key == -3);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_24---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 4, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 3, "empty");
        insert_rbt(&root, -5, "empty");
        remove_rbt(&root, 4);
        CU_ASSERT(root->key == 3);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_25---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, -5, "empty");
        insert_rbt(&root, 2, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, -1, "empty");
        remove_rbt(&root, 5);
        remove_rbt(&root, -5);
        CU_ASSERT(root->key == 0);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_26---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 3, "empty");
        insert_rbt(&root, 0, "empty");
        insert_rbt(&root, 6, "empty");
        insert_rbt(&root, -4, "empty");
        insert_rbt(&root, 2, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, -6, "empty");
        insert_rbt(&root, -1, "empty");
        remove_rbt(&root, 5);
        remove_rbt(&root, 6);
        CU_ASSERT(root->key == 0);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_27---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, -2, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, -5, "empty");
        insert_rbt(&root, -1, "empty");
        insert_rbt(&root, 3, "empty");
        insert_rbt(&root, -6, "empty");
        insert_rbt(&root, 0, "empty");
        remove_rbt(&root, -5);
        remove_rbt(&root, -6);
        CU_ASSERT(root->key == 1);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_28---------------------------\n\n");
        root = NULL;
        int tree_arr[] = {-1, -3, 2, -5, -2, 1, 8, 6, -4};
        for (size_t i = 0; i < 9; i++) {
            insert_rbt(&root, tree_arr[i], "empty");
        }
        root->left->color = BLACK;
        root->right->color = BLACK;
        remove_rbt(&root, 1);
        /* free_tree(root); */
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        free_tree(root);

        printf("\n\n----------------------case_29---------------------------\n\n");
        root = NULL;
        int arr_nodes[] = {7, 4, 13, 2, 5, 10, 17, 1, 9, 11, 15, 18, 12};
        for (size_t i = 0; i < 13; i++) {
            insert_rbt(&root, arr_nodes[i], "empty");
        }
        remove_rbt(&root, 5);
        CU_ASSERT(is_rb_tree(root, &issue) == 1);
        /* free_tree(root); */
    }

        printf("\n\n----------------------case_15---------------------------\n\n");
        root = NULL;
        insert_rbt(&root, 10, "empty");
        insert_rbt(&root, 5, "empty");
        insert_rbt(&root, 30, "empty");
        insert_rbt(&root, 1, "empty");
        insert_rbt(&root, 7, "empty");
        insert_rbt(&root, 25, "empty");
        insert_rbt(&root, 40, "empty");
        insert_rbt(&root, 8, "empty");
        insert_rbt(&root, 6, "empty");
        root->left->color = BLACK;
        root->right->right->color = BLACK;
        root->right->left->color = BLACK;
        root->left->right->color = RED;
        root->left->right->left->color = BLACK;
        root->left->right->right->color = BLACK;
        /* insert_rbt(&root, 50); */
        /* insert_rbt(&root, 35); */
        remove_rbt(&root, 40);
        /* CU_ASSERT(root->key == 7); */
        /* CU_ASSERT(root->color == BLACK); */
        /* CU_ASSERT(root->right->key == 10); */
        /* CU_ASSERT(root->right->color == BLACK); */
        /* CU_ASSERT(root->left->key == 5); */
        /* CU_ASSERT(root->left->color == BLACK); */
        /* CU_ASSERT(root->right->right->key == 30); */
        /* CU_ASSERT(root->right->right->color == BLACK); */
        /* CU_ASSERT(root->right->left->key == 8); */
        /* CU_ASSERT(root->right->left->color == BLACK); */
        /* CU_ASSERT(root->left->left->key == 1); */
        /* CU_ASSERT(root->left->left->color == BLACK); */
        /* CU_ASSERT(root->left->right->key == 6); */
        /* CU_ASSERT(root->left->right->color == BLACK); */
        /* CU_ASSERT(root->right->right->left->key == 25); */
        /* CU_ASSERT(root->right->right->left->color == RED); */
        /* CU_ASSERT(is_rb_tree(root, &issue) == 1); */
        /* free_tree(root); */

    tree_to_dot(root);

    free_tree(root);
}

int main() {
    CU_initialize_registry();
    CU_pSuite suite = CU_add_suite("Tree Suite", NULL, NULL);
    CU_add_test(suite, "test of remove_rbt()", test_remove_rbt);

    CU_basic_set_mode(CU_BRM_VERBOSE);
    CU_basic_run_tests();
    CU_cleanup_registry();

    return 0;
}
